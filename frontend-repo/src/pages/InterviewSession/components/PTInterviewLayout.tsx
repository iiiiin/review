...
export default function PTInterviewLayout({ sessionId, initialAttemptIds = [], onComplete }: PTLayoutProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        console.log('🔄 [PT] 리트라이 모드: 기존 문제 정보 복원 시작');
        ...
        console.log('📥 [PT] 기존 피드백 데이터:', feedback);
        ...
        console.log('📥 [PT] 리트라이 API 응답:', retryData);
        ...
        if (retryData?.ptInterview?.[0]) {
          ...
          console.log('✅ [PT] 리트라이 모드 데이터 설정 완료:', data);
          dispatch({ type: 'SET_DATA', payload: data });
          setHasInitialized(true);
          
          // 새로운 세션에 조인
          if (newAttempt.id) {
            console.log('🔄 [PT] 리트라이 모드: 새로운 세션 조인 (attemptId:', newAttempt.id, ')');
  ...
  useEffect(() => {
    ...
    console.log('🚀 [PT] 일반 모드: sessionStorage에서 데이터 로드');
    setIsSessionConnecting(true);
  ...
  const fallbackToNormalMode = () => {
    ...
    try {
      ...
      console.log('✅ [PT] 일반 모드 데이터 설정 완료:', data);
      dispatch({ type: 'SET_DATA', payload: data });
    } catch (e) {
  ...
  useEffect(() => {
    ...
    console.log('🔄 [PT] 세션 조인 시작 (sessionId:', openViduSessionId, ')');
    setIsSessionConnecting(true);
    
    // 실제 세션 조인 호출
    joinSession(openViduSessionId).then(() => {
      console.log('✅ [PT] 세션 조인 성공');
      setIsSessionConnecting(false);
    }).catch((error) => {
  ...
  useEffect(() => {
    return () => {
      if (session) {
        console.log('🔌 [PT] 컴포넌트 언마운트 - 세션 정리');
        try {
          leaveSession();
        } catch (error) {
  ...
  const startRecordingMutation = useMutation({
    ...
    onSuccess: (data: any) => {
      ...
      if (newRecordingId) {
        ...
        console.log('✅ PT 녹화 시작 성공:', newRecordingId);
      }
  ...
  const stopRecordingMutation = useMutation({
    ...
    onSuccess: () => console.log('✅ PT 녹화 중지 성공'),
  ...
  const handlePresentationFinish = useCallback(async () => {
    try {
      console.log('🎬 [PT] 발표 완료 처리 시작');
      
      // 녹화 중지
      if (recordingId) {
        console.log('🎬 [PT] 녹화 중지 중...');
        await stopRecordingMutation.mutateAsync(recordingId);
        console.log('✅ [PT] 녹화 중지 완료');
      }
      
      // WebSocket 진행 상태: 질문 완료 처리 (서버가 answerAttemptId를 전송하도록 트리거)
      console.log('📡 [PT] WebSocket 질문 완료 알림 전송');
      wsCompleteQuestion();
      
      // 세션 나가기 (리트라이 모드에서도 안전하게 처리)
      console.log('🔌 [PT] 세션 나가기 시작');
      if (session) {
        await leaveSession();
        console.log('✅ [PT] 세션 나가기 완료');
      } else {
        console.log('⚠️ [PT] 세션이 이미 종료된 상태');
      }
      
      // 상위 컴포넌트에 완료 알림 (InterviewSession에서 WebSocket 처리 및 CompletionScreen 렌더링)
      if (onComplete) {
        console.log('📤 [PT] 상위 컴포넌트에 완료 알림 전송');
        onComplete([]);
      }
      
      // 로컬 상태도 complete로 변경
      console.log('🔄 [PT] 상태를 complete로 변경');
      ...
      try {
        if (session) {
          await leaveSession();
          console.log('✅ [PT] 오류 발생 후 세션 정리 완료');
        }
      } catch (leaveError) {
  ...
  useEffect(() => {
    ...
    console.log('🔍 [PT 녹화시작체크] phase:', phase, 'session:', !!session, 'publisher:', !!publisher, 'recordingId:', recordingId);
    
    if (phase === 'waiting_presentation' && session && publisher && !recordingId && !startRecordingMutation.isPending) {
      console.log('🎥 PT 녹화 대기 중 녹화 시작!');
      startRecordingMutation.mutate(session.sessionId);
    } else {
      console.log('🔍 [PT 녹화시작체크] 조건 미충족 - 녹화 시작하지 않음');
    }
  ...
  if (phase === 'loading') {
    // PT 데이터가 로드되었는지 확인
    if (ptData) {
      console.log('✅ [PT] 데이터 로드 완료, 로딩 상태 해제');
