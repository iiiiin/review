...
export const useWebSocketConnection = ({
  ...
  const connect = useCallback(async () => {
    try {
      ...
      console.log('📊 면접 정보:');
      console.log(`  - 예상 총 질문 수: ${totalQuestions}`);
      console.log('📞 handleAnalysisResult 콜백 함수:', typeof onAnalysisResultRef.current);
  ...
  useEffect(() => {
    if (shouldConnect && !isConnected && onAnalysisResultRef.current) {
      console.log('🔌 자동 WebSocket 연결 조건 충족');
      connect();
    }
  ...
  useEffect(() => {
    return () => {
      console.log('🔌 WebSocketConnection 훅 정리 - 콜백 유지 (분석 결과 수신 대기)');
