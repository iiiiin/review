...
const GeneralInterviewContent: React.FC<{
  ...
  const { state, actions, currentQuestion, isLastQuestion, currentSet } = interviewState;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        console.log('🔄 [GeneralInterview] 리트라이 모드: 기존 세션 정리 및 새로운 세션 시작');
        
        // 리트라이 모드에서는 기존 세션을 정리
        if (session.session) {
          console.log('🔄 [GeneralInterview] 리트라이 모드: 기존 세션 정리');
        ...
        setTimeout(() => {
          if (targetIds.length > 0) {
            console.log('🔄 [GeneralInterview] 리트라이 모드: 새로운 세션 시작');
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.log('>>> [GeneralInterview] generateNext 감지 - 다음 질문 세트 생성 및 로드 시작');
        ...
        console.log(`>>> [GeneralInterview] 새 질문 세트 로드: ${expandedNewQuestions.length}개`);
  ...
  const handleAnalysisResult = useCallback((answerAttemptId: string) => {
    console.log('📊 [GeneralInterview] answerAttemptId 수집:', answerAttemptId);
  ...
  const handleNextSet = useCallback(() => {
    ...
    if (nextSetIndex < effectiveTotalSets) {
      ...
      console.log(`📈 다음 세트로 이동: 세트 ${nextSetIndex + 1}, 질문 ${nextQuestionIndex + 1}`);
    } else {
      // 마지막 세트인 경우 면접 완료
      actions.complete();
      setShowSetFeedback(null);
  ...
  const handleRetrySet = useCallback(() => {
    ...
    console.log(`🔄 세트 재시도: 세트 ${currentSetIndex + 1}, 질문 ${firstQuestionOfSet + 1}부터 재시작`);
  ...
  const handleNextQuestion = async () => {
    ...
    const processNextQuestion = async () => {
      ...
      if (isLastQuestionInCurrentSet) {
        console.log(`🎬 세트 ${currentSet + 1} 종료 - 세션 나가기`);
        if (session.session) {
          try {
            await session.endSession();
          } catch (err) {
  ...
  useEffect(() => {
    ...
    console.log('📡 WebSocket Context 연결 완료');
    console.log('📡 분석 결과 핸들러:', typeof handleAnalysisResult);
    
    // websocket Context에서 이미 연결 관리를 하므로 추가 설정 불필요
    return () => {
      console.log('📡 WebSocket 결과 핸들러 정리');
    };
  ...
  useEffect(() => {
    ...
    if (isRetryMode) {
      console.log(`🔄 [GeneralInterview] 리트라이 모드 세션 변경: ${session.currentSessionId} -> ${newSessionId}`);
      // 기존 세션과 다른 경우에만 변경
      ...
      console.log(`🔄 [GeneralInterview] 일반 모드 세션 변경: ${session.currentSessionId} -> ${newSessionId}`);
      // sessionId에서 ~숫자 부분 제거
      const cleanSessionId = newSessionId.includes('~') ? newSessionId.split('~')[0] : newSessionId;
      session.changeSession(cleanSessionId);
    }
  ...
  useEffect(() => {
    if (step === 'waiting_recording' && session.isSessionReady && questions.length > 0) {
      ...
      if (questionId && !recording.isRecording && !recording.isStarting) {
        console.log('🎥 [GeneralInterview] 녹화 시작 조건 충족');
  ...
  useEffect(() => {
    ...
    if (shouldConnect) {
      const totalQuestionsCount = totalSets * 3; // 인성/직무는 각 InterviewSet당 3개 질문
      console.log(`📊 [GeneralInterview] 면접 정보: ${interviewType}, 총 ${totalQuestionsCount}개 질문`);
      websocket.connect();
    }
...
export default function GeneralInterviewSession({ interviewType, sessionId, initialAttemptIds }: GeneralInterviewSessionProps) {
  ...
  // WebSocket 분석 결과 핸들러 ✅
  const handleAnalysisResult = useCallback((answerAttemptId: string) => {
    console.log('📊 [Provider] WebSocket 분석 결과 수신:', answerAttemptId);
    
    // 실제 분석 결과 처리는 GeneralInterviewContent에서 구현됨
    // Provider에서는 WebSocket Context로 전달하는 역할만 수행
    console.log('📊 분석 결과를 Context를 통해 전달 완료:', answerAttemptId);
  }, []);
