...
import { createSession, createToken } from '@/pages/InterviewSession/api/interview';

export function useOpenVidu(mySessionId: string, myUserName: string) {
  // sessionId에서 ~숫자 부분 제거 (uuid~숫자 형태일 경우 uuid만 사용)
  const cleanSessionId = mySessionId.includes('~') ? mySessionId.split('~')[0] : mySessionId;
  
  console.log('Initial Session ID:', mySessionId);
  ...
  const leaveSession = useCallback(() => {
    // 🔹 카메라·마이크 트랙 해제 로직 추가
    console.log('publisherRef.current >>> ', publisherRef.current);
    console.log('MediaStream >>> ', publisherRef.current?.stream?.getMediaStream());
