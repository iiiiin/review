...
CREATE TABLE `script_file` (
  `script_file_uuid` varchar(36) NOT NULL,
  `enterprise_name` varchar(100) NOT NULL,
  `file_name` varchar(255) NOT NULL,
  `ocr` longtext NOT NULL,
  `position` varchar(100) NOT NULL,
  `script_uploaded_at` datetime(6) NOT NULL,
  `script_url` varchar(500) NOT NULL,
  `user_uuid` varchar(36) NOT NULL,
  PRIMARY KEY (`script_file_uuid`),
  KEY `FK2x35huvldi955v9gu475cynca` (`user_uuid`),
  CONSTRAINT `FK2x35huvldi955v9gu475cynca` FOREIGN KEY (`user_uuid`) REFERENCES `users` (`user_uuid`)
