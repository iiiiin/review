...
import { startRecording, stopRecording } from '@/pages/InterviewSession/api/interview';
import ProgressBar from './ProgressBar';
import CompletionScreen from './CompletionScreen';
import PTInterviewLayout from './PTInterviewLayout';
import SetFeedbackModal from './SetFeedbackModal';

import { motion } from 'framer-motion';
import { Video, Loader, Timer } from 'lucide-react';
import UserVideo from './UserVideo';
import { useOpenVidu } from '@/pages/InterviewSession/hooks/useOpenVidu';
...
export default function InterviewSession({ interviewType, sessionId, initialAttemptIds }: InterviewSessionProps) {
  ...
  useEffect(() => {
    if (initialAttemptIds && initialAttemptIds.length > 0) {
      console.log('🔁 재시도 진입 - 전달된 attemptIds:', initialAttemptIds);
    }
  }, [initialAttemptIds]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.log('🔄 재시도 모드 - targetIds 순서:', targetIds);
        ...
        const items: ExpandedQ[] = feedbackResults.map((item, idx) => {
          ...
          console.log(`질문 ${idx + 1} 구성 - ID: ${item.id}, 질문: ${feedback?.question || feedback?.title || '기본값'}`);
    ...
    if (raw) {
      try {
        ...
        if (typeof saved?.totalInterviewSets === 'number' && saved.totalInterviewSets > 0) {
          console.log('🧭 저장된 totalInterviewSets 복원:', saved.totalInterviewSets);
    ...
    const timer = setTimeout(() => {
      ...
      if (storeKey) {
        ...
        if (raw) {
          try {
            const saved = JSON.parse(raw);
            if (Array.isArray(saved?.questions) && saved.questions.length > 0) {
              console.log('>>> [Session] 세션스토리지에서 질문 발견 - 바로 로드');
              const expanded = expandQuestions(toFullQs(saved.questions));
              dispatch({ type: 'LOAD_QUESTIONS', payload: expanded });
      ...
      (async () => {
        try {
          ...
          console.log('>>> [Session] 세션스토리지에 질문 없음 - API 호출');
          ...
          console.log(`>>> [Session] 새 질문 세트 로드: ${expandedNewQuestions.length}개`);
  ...
  const handleAnalysisResult = useCallback((answerAttemptId: string) => {
    console.log('📊 answerAttemptId 수집:', answerAttemptId);
    
    // 전체 answerAttemptIds 업데이트
    setAnswerAttemptIds(prev => {
      const newIds = !prev.includes(answerAttemptId) ? [...prev, answerAttemptId] : prev;
      console.log('📊 현재 수집된 answerAttemptIds:', newIds);
      console.log(`📊 수집 진행률: ${newIds.length}/${effectiveTotalSets * 3} (${Math.round(newIds.length / (effectiveTotalSets * 3) * 100)}%)`);
      return newIds;
    });
    ...
    setSetAnswerAttempts(prev => {
      ...
      const targetSet = Math.floor(totalCollected / 3);
                ...
                console.log(`📊 분석 결과 추가: InterviewSet ${targetSet + 1}, 총 수집된 수: ${totalCollected + 1}/${effectiveTotalSets * 3}`);
      ...
      if (!currentSetAttempts.includes(answerAttemptId)) {
        ...
        console.log(`📊 세트 ${targetSet} answerAttemptIds:`, newSetAttempts);
        
        // 첫 번째 피드백이 도착하면 기록 (3개 질문 모두 완료 후에만 페이지 이동)
        if (newSetAttempts.length === 1) {
          console.log(`🎯 InterviewSet ${targetSet + 1} 첫 번째 피드백 도착!`);
          ...
          if ((!isQuestionInProgress && isCurrentSetComplete) || showSetFeedback === targetSet) {
            if (showSetFeedback === targetSet) {
              console.log('🚀 모달 대기 중 첫 번째 피드백 도착 - 피드백 페이지로 즉시 이동');
              setShowSetFeedback(null); // 모달 닫기
            } else {
              console.log('✅ 현재 세트 모든 질문 완료됨 - 피드백 페이지로 이동');
            }
            navigateToFeedbackPage(newSetAttempts);
          } else {
            console.log('⏳ 아직 세트 질문 진행 중 - 피드백 페이지 이동 지연');
            console.log(`  - 질문 진행 상태: ${step}, 현재 질문: ${currentQuestionIndex}, 세트 완료 필요: ${(currentSet + 1) * 3 - 1}`);
          }
        }
        // 추가 피드백 도착 로깅 (페이지에서 실시간 업데이트됨)
        else {
          console.log(`📊 InterviewSet ${targetSet + 1} 피드백 ${newSetAttempts.length}/3 수집됨`);
          if (newSetAttempts.length === 3) {
            console.log(`🎉 InterviewSet ${targetSet + 1} 모든 피드백 완료!`);
          }
        }
  ...
  useEffect(() => {
    ...
    if (shouldConnectWebSocket) {
      console.log('🔌 면접 시작 - 전역 WebSocket 연결');
      console.log('📞 handleAnalysisResult 콜백 함수:', typeof handleAnalysisResult);
      ...
      console.log(`📊 면접 정보:`);
      console.log(`  - 원본 면접 타입: ${interviewType}`);
      console.log(`  - 정규화된 면접 타입: ${normalizedInterviewType}`);
      console.log(`  - 사용자 설정 InterviewSet 수: ${totalSets}`);
      console.log(`  - 예상 총 질문 수: ${totalQuestionsCount}`);
      console.log(`  - 현재 로드된 질문 수: ${questions.length}`);
      
      startInterview(totalQuestionsCount);
      console.log('🚀 WebSocket 연결 요청 시작...');
    ...
    return () => {
      console.log('🔌 InterviewSession 언마운트 - 콜백 유지 (분석 결과 수신 대기)');
  ...
  useEffect(() => {
    ...
    const sessionManager = async () => {
      try {
        ...
        if (session) {
          console.log('🔌 기존 세션 종료 (useEffect)');
          await leaveSession();
        }
  ...
  useEffect(() => {
    console.log('🔍 [녹화시작체크] step:', step, 'session:', !!session, 'publisher:', !!publisher, 'questions:', questions.length);
    ...
    if (step === 'waiting_recording' && session && publisher && questions.length > 0) {
      ...
      console.log('🔍 [녹화대기→시작] sessionId:', sessionId, 'questionId:', questionId, 
                  'recordingStarted:', recordingStarted[sessionId], 'currentRecordingId:', currentRecordingId);
      ...
      if (questionId && !recordingStarted[sessionId] && !currentRecordingId && !startRecordingMutation.isPending) {
        console.log('🎥 녹화 대기 중 녹화 시작! sessionId:', sessionId, 'questionId:', questionId);
        setRecordingStarted(prev => ({ ...prev, [sessionId]: true }));
        startRecordingMutation.mutate({
          interviewId: sessionId,
        });
      } else {
        console.log('🔍 [녹화대기→시작] 조건 미충족 - 녹화 시작하지 않음');
      }
    }
  ...
  const handleRetrySet = () => {
    ...
    console.log(`🔄 세트 ${setIndex + 1} 리트라이 시작`);
    ...
    console.log(`🔄 세트 ${setIndex + 1} 질문 ${targetQuestionIndex + 1}로 이동`);
  };
  ...
  const generateNextQuestionSet = async () => {
    ...
    try {
      console.log(`>>> [API] 다음 질문 세트 생성 요청 - interviewUuid: ${interviewUuid}`);
      ...
      console.log(`>>> [API] 다음 질문 세트 생성 완료:`, response);
      ...
      if (Array.isArray(newQuestions) && newQuestions.length > 0) {
        const expandedNewQuestions = expandQuestions(toFullQs(newQuestions));
        console.log(`>>> [API] 새로운 질문 ${expandedNewQuestions.length}개 추가`);
        console.log('>>> [API] 확장된 질문 상세:', expandedNewQuestions);
  ...
  const handleNextSetFromModal = async () => {
    ...
    console.log(`🚀 다음 세트로 진행: 세트 ${nextSetIndex + 1}`);
    
    if (nextSetIndex < totalSets) {
      // 다음 질문 세트 생성 API 호출
      try {
        console.log('>>> [Modal] 다음 질문 세트 생성 요청');
        await generateNextQuestionSet();
        
        console.log(`>>> [Modal] 질문 생성 완료 - 세트 ${nextSetIndex + 1}로 이동`);
        ...
        setTimeout(() => {
          ...
          console.log(`>>> [Modal] 세트 ${nextSetIndex + 1} 질문 ${nextQuestionIndex + 1}로 이동 완료`);
        }, 100);
  ...
  const handleNextQuestion = useCallback(() => {
    if (stopRecordingMutation.isPending) {
      console.log('🚫 녹화 중지 진행 중 - 중복 요청 방지');
      return;
    }
    
    // 추가 안전 장치: 이미 처리 중인 경우 방지
    if (step === 'complete') {
      console.log('🚫 이미 면접 완료 상태 - 중복 처리 방지');
      return;
    }
    ...
    const processNextQuestion = async () => {
      ...
      if (isLastQuestionInSet && session) {
        console.log(`🎬 세트 ${currentSet + 1} 종료 - 세션 나가기 (processNextQuestion)`);
        try {
          await leaveSession(); // ✅ 세트 종료 시점 sessionLeave
          console.log('🔌 세션 종료 완료 (세트 종료)');
        } catch (err) {
  ...
  const handlePTComplete = useCallback((ptAnswerAttemptIds: string[]) => {
    console.log('✅ [PT] PT 완료 콜백 수신:', ptAnswerAttemptIds);
    console.log('📊 [PT] 현재 수집된 answerAttemptIds:', answerAttemptIds);
  ...
  if (step === 'complete') {
    console.log('🔍 InterviewSession - CompletionScreen으로 전달하는 데이터:');
    console.log('  - resultId:', resultId);
    console.log('  - 원본 interviewType:', interviewType);
    console.log('  - 정규화된 interviewType:', normalizedInterviewType);
